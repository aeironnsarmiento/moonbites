...
def test_advance_claims_lease_and_returns_pending_snapshot():
    _authenticated()
    job = _job(version=1)
    claimed = _job(version=2, lease_token="lease-abc")
    released = _job(version=3)
    try:
        with (
            patch(
            ...
            patch(
                "backend.app.api.routes.import_jobs.checkpoint_job",
    ...
    assert checkpoint.call_args.kwargs["release_lease"] is True
