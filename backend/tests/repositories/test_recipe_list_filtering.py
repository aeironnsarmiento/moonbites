...
from app.schemas.extract import NormalizedRecipe, RecipeSortOption
...
def test_build_paginated_response_keeps_total_count_and_pages_from_sql():
    raw_records = [_record_dict("1"), _record_dict("2")]

    response = _build_paginated_response(raw_records, page=2, page_size=5, total_count=12)
...
def test_build_paginated_response_warns_when_dedupe_removes_a_row(caplog):
    # Two rows sharing a source identity should never reach the database --
    # the unique constraint on submitted_url_canonical/final_url_canonical
    # prevents it. If it happens anyway (a bypassed or dropped constraint),
    # the response must not silently ship a short page with a stale
    # total_pages; it must say so.
    duplicate = _record_dict("1")
    duplicate["id"] = "2"
    raw_records = [_record_dict("1"), duplicate]

    with caplog.at_level("WARNING", logger="app.repositories.recipe_imports"):
        response = _build_paginated_response(
            raw_records, page=1, page_size=10, total_count=2
        )

    assert [item.id for item in response.items] == ["1"]
    assert response.total_count == 2
    assert any(
        "dedupe removed" in record.message for record in caplog.records
    )


def test_build_paginated_response_no_warning_when_rows_are_unique(caplog):
    raw_records = [_record_dict("1"), _record_dict("2")]

    with caplog.at_level("WARNING", logger="app.repositories.recipe_imports"):
        _build_paginated_response(raw_records, page=1, page_size=10, total_count=2)
